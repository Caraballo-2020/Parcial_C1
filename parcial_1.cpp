#include <iostream>
#include <cmath>
#include <cstdlib> // Librería para usar exit() y así evitar errores posteriores
...
{
    char respuesta;
    char coma; // Para leer la coma en la respuesta del usuario
    ...
    if (respuesta == 's' || respuesta == 'S')
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (std::cin.fail() || coma != ',') // Para evitar error al ingresar dato incorrecto
            {
                std::cout << "Punto ingresado no válido" << std::endl;
                exit(1);
            }
        }
    }
    ...
    {
        // Si la cantidad de puntos a ingresar es diferente a 4, habrá error, se evitará de la siguiente manera:
        if (n == 4)
        {
        ...
        {
            std::cout << "Si usa los datos predeterminados, ingresar un número de puntos igual a 4" << std::endl;
            exit(1); // Salir del programa para evitar errores
        }
    }
}
...
{
    Point pi = puntos[0];
    double distanciaMinima = calcularDistancia(pi, pUsuario);
    double distancia;
    for (int i = 0; i < n; i++)
    {
        pi = puntos[i];
        distancia = calcularDistancia(pi, pUsuario);
...
double arregloPuntos(Point puntos[], int n)
{
    ...
    for (int i = 0; i < n - 1; i++)
    {
        aux = calcularDistancia(puntos[i], puntos[i + 1]); // Se calcula la distancia entre el punto actual y el siguiente
        res = res + aux;                                   // Se van acumulando las distancias en el resultado
    }
    return res;
}
...
int main()
{
    char coma; // Para leer la coma en el punto del usuario
    int n;
    ...
    // Bono:
    char bono;
    std::cout << "¿Desea calcular la distancia total de los puntos que conforman el arreglo al recorrerlos en orden? (s/n): ";
    std::cin >> bono;
    if (bono == 's' || bono == 'S')
    {
